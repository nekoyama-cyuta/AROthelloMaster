...
namespace Unity.XR.XREAL
{
    ...
    public class XREALRGBCameraTexture : SingletonMonoBehaviour<XREALRGBCameraTexture>
    {
        ...
        /// <summary>Triggered when new native camera YUV420 plane pointers are available: (YData, UData, VData, width, height).</summary>
        public static Action<IntPtr, IntPtr, IntPtr, int, int> OnNativeCameraPlanesAvailable;
        ...
        private unsafe void LoadYUVFormatTexture(int frameHandle)
        {
            ...
            // Notify native consumers (e.g. OpenCV bridge) with direct hardware pointers
            OnNativeCameraPlanesAvailable?.Invoke(YData, UData, VData, sizeY.x, sizeY.y);
        }
    }
}
